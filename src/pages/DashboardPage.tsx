...
import {
  ...
  hotelGoalsAtom,
  dashboardDataAtom,
  lastYearSalesDetailsForSelectedHotelAtom,
  lastYearSalesDetailsForAllHotelsAtom,

  currentMonthGroupTotalsAtom,
  currentHotelDetailsAtom,
  currentHotelGoalAtom,
  currentMonthAllDetailsAtom,

  loadDashboardData,
  loadHotels,
  loadSalesDetails,
  loadHotelGoals,
  loadLastYearSalesDetailsForAllHotels,
...
import GroupObjectivesRow from '../components/GroupObjectivesRow';
import { TrendingUp, BarChart4, Plus, RefreshCw, Target, Trophy, Award, Medal } from 'lucide-react';
...
const getRankIcon = (index: number) => {
    if (index === 0) return <Trophy className="w-5 h-5 text-yellow-500 flex-shrink-0" />;
    if (index === 1) return <Award className="w-5 h-5 text-gray-400 flex-shrink-0" />;
    if (index === 2) return <Medal className="w-5 h-5 text-orange-500 flex-shrink-0" />;
    return <span className="font-semibold text-gray-500 w-5 text-center flex-shrink-0">{index + 1}</span>;
};
...
const DashboardPage = () => {
  ...
  const [, setLastYearSalesDetailsForSelectedHotel] = useAtom(lastYearSalesDetailsForSelectedHotelAtom);
  const [lastYearSalesDetailsForAllHotels, setLastYearSalesDetailsForAllHotels] = useAtom(lastYearSalesDetailsForAllHotelsAtom);
  ...
  const fetchData = async (refresh = false) => {
    ...
    setError(null);
    try {
      const [hotelsData, baseData, detailsData, goalsData, lastYearDetailsData] = await Promise.all([
        loadHotels(),
        loadDashboardData(),
        loadSalesDetails(),
        loadHotelGoals(),
        loadLastYearSalesDetailsForAllHotels(selectedDate.month, selectedDate.year)
      ]);
      setHotels(hotelsData);
      setDashboardData(baseData);
      setSalesDetails(detailsData);
      setHotelGoals(goalsData);
      setLastYearSalesDetailsForAllHotels(lastYearDetailsData);
    } catch (err: any) {
  ...
  useEffect(() => {
      if (selectedHotel) {
        const hotelData = lastYearSalesDetailsForAllHotels.find(d => d.hotelId === selectedHotel);
        setLastYearSalesDetailsForSelectedHotel(hotelData || null);
      } else {
        setLastYearSalesDetailsForSelectedHotel(null);
      }
  }, [selectedHotel, lastYearSalesDetailsForAllHotels, setLastYearSalesDetailsForSelectedHotel]);
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6">
        ...
        {isLoading && !isInitializing ? (
          ...
          <>
            ...
            {/* Se nenhum hotel estiver selecionado (Visão de Grupo) */}
            {!selectedHotel && (
              <div className="space-y-6">
                <div className="dashboard-card">
                    ...
                    <DistributionChart
                    salesDetails={allDetailsForMonth}
                    hotels={hotels}
                    />
                </div>
            ...
            {/* Se um hotel estiver selecionado - LAYOUT INDIVIDUAL RESTAURADO */}
